...
const visualPercent = Number(usedPercentage.toFixed(1));
  ...
  // ===== RANK LOGIC (FIXED) =====
  // UserRank = current rank the user is at (may or may not be qualified yet)
  // NextRank = the next rank the user is aiming for
  // If UserRank === NextRank → user hasn't qualified yet → 0% progress, no tick
  const userRank = data?.UserRank || "Associate"
  const nextRank = data?.NextRank || "Associate"

  // Index of NextRank in the RANKS ladder
  const nextRankIndex = (() => {
    const idx = RANKS.findIndex(r => r.n.toLowerCase() === String(nextRank).toLowerCase())
    return idx >= 0 ? idx : 0
  })()

  // If UserRank and NextRank are the same → user hasn't qualified yet → 0%
  const isSameRank = String(userRank).toLowerCase() === String(nextRank).toLowerCase()
  const rankPct = isSameRank
    ? 0
    : Math.round((nextRankIndex / (RANKS.length - 1)) * 100)

  
  const liveRankIndex = nextRankIndex

  useEffect(() => {
    const userRankIndex = RANKS.findIndex(
      (rankItem) => rankItem.n.toLowerCase() === String(data?.UserRank || "Associate").toLowerCase()
    )
    setSelectedRank(userRankIndex >= 0 ? userRankIndex : 0)
  }, [data?.UserRank])
                ...
                <div className="relative shrink-0 w-24 h-24">
                  <svg width="96" height="96" viewBox="0 0 96 96" className="-rotate-90">
                    ...
                    <circle
                      ...
                      stroke={C.green} strokeLinecap="round"
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <b className="text-[19px]" style={{ fontFamily: DISPLAY_FONT, color: C.green }}>{rankPct}%</b>
                        ...
                        {i > 0 && (
                          <div
                            ...
                            style={{
                              ...
                              background: i <= liveRankIndex ? C.green : "#d5deee",
                            }}
                          />
                        )}
                        <div onClick={() => setSelectedRank(i)} className="flex flex-col items-center cursor-pointer w-[92px]">
                          <div
                            ...
                            style={{
                              ...
                              background: done
                                ? `linear-gradient(135deg, ${C.green}, #1fa864)`
                                : current
                                ? `radial-gradient(circle, ${C.blue500}, ${C.navy700})`
                                : "#eef2fa",
                              border: `2px solid ${done ? C.green : current ? C.blue400 : "#c7d1e6"}`,
                              color: done ? "#fff" : current ? "#fff" : C.mute,
                              boxShadow: current
                          ...
                          <div
                            className={`font-semibold text-center text-xs mt-2.5 ${
                              done ? "text-[#2fbf7a]" : current ? "text-[#2f6bff]" : "text-[#0c1c3d]"
                            }`}
                          >
                            {r.n}
                          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className={`${CARD_CLS} p-6`}>
                ...
                <div className="grid grid-cols-2 gap-3.5">
                  ...
                  <TrItem label="STRONG LEG" value={`$${Number(otherLegBusiness).toLocaleString()}`} />
              ...
              <div className={`${CARD_CLS} flex flex-col items-center justify-center p-6 gap-3.5`}>
                ...
                <div className="relative w-[140px] h-[140px]">
                  <svg width="140" height="140" viewBox="0 0 140 140" className="-rotate-90">
                    ...
                    <circle cx="70" cy="70" r="58" stroke="#74c0fc" strokeWidth="16" fill="none" strokeDasharray="123.9 240.5" strokeDashoffset="0" strokeLinecap="round" />
                    <circle cx="70" cy="70" r="58" stroke="#5a9fd4" strokeWidth="16" fill="none" strokeDasharray="123.9 240.5" strokeDashoffset="-123.9" strokeLinecap="round" />
                    <circle cx="70" cy="70" r="58" stroke="#e0ac2e" strokeWidth="16" fill="none" strokeDasharray="116.6 247.8" strokeDashoffset="-247.8" strokeLinecap="round" />
                  </svg>
                </div>
                <div className="flex gap-[18px]">
                  <div className="flex items-center gap-1.5 text-[11px] text-[#4c5b7c]">
                    <span className="rounded-full w-2 h-2 bg-[#74c0fc]" />Strong Leg 34%
                  </div>
                  <div className="flex items-center gap-1.5 text-[11px] text-[#4c5b7c]">
                    <span className="rounded-full w-2 h-2 bg-[#5a9fd4]" />Second leg 34%
                  </div>
                  <div className="flex items-center gap-1.5 text-[11px] text-[#4c5b7c]">
                    <span className="rounded-full w-2 h-2 bg-[#e0ac2e]" />Other Leg 32%
                  </div>
                </div>
              </div>
            </div>
...
function StatMini({ label, value, color }) {
  return (
    <div>
      ...
      <b className="text-[15px] dark:!text-[#e0ac2e]" style={{ fontFamily: DISPLAY_FONT, color: color || C.ink }}>{value}</b>
    </div>
  )
}
...
function RankDetailItem({ label, value, color }) {
  return (
    <div>
      ...
      <b className="text-[15px] dark:!text-white" style={{ fontFamily: DISPLAY_FONT, color: color || C.ink }}>{value}</b>
    </div>
  )
}
...
function StatTile({ value, label, color }) {
  return (
    <div className={`${CARD_CLS} text-center p-6`}>
      <div className="font-bold text-2xl dark:!text-white" style={{ fontFamily: DISPLAY_FONT, color: color || C.ink }}>{value}</div>
