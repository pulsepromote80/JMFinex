...
// /* =========================
//    LIVE MARKET HOOK (with stable prices + weekend caching)
// ========================= */
...
//   // ===== Stable market price builder (no random per-render) =====
...
//     // Default stable change values (used if real API change not available)
//     const defaultChanges = {
//       "EUR/USD": { change: "0.05", positive: false },
//       "GBP/USD": { change: "0.06", positive: true },
//       "XAU/USD": { change: "0.05", positive: false },
//       "USD/JPY": { change: "0.00", positive: true },
//     };

//     // Friday -> Saturday transition: cache Friday's prices
//     if (
//       isWeekendDay &&
...
//         "EUR/USD": {
//           price: bases["EUR/USD"],
//           change: realChangesRef.current["EUR/USD"]?.change ?? defaultChanges["EUR/USD"].change,
//           positive: realChangesRef.current["EUR/USD"]?.positive ?? defaultChanges["EUR/USD"].positive,
//         },
//         "GBP/USD": {
//           price: bases["GBP/USD"],
//           change: realChangesRef.current["GBP/USD"]?.change ?? defaultChanges["GBP/USD"].change,
//           positive: realChangesRef.current["GBP/USD"]?.positive ?? defaultChanges["GBP/USD"].positive,
//         },
//         "XAU/USD": {
//           price: bases["XAU/USD"],
//           change: realChangesRef.current["XAU/USD"]?.change ?? defaultChanges["XAU/USD"].change,
//           positive: realChangesRef.current["XAU/USD"]?.positive ?? defaultChanges["XAU/USD"].positive,
//         },
//         "USD/JPY": {
//           price: bases["USD/JPY"],
//           change: realChangesRef.current["USD/JPY"]?.change ?? defaultChanges["USD/JPY"].change,
//           positive: realChangesRef.current["USD/JPY"]?.positive ?? defaultChanges["USD/JPY"].positive,
//         },
//       };
//       console.log("📅 Weekend started - Cached Friday's prices");
//     }
...
//         },
//       };
//       console.log("📅 Friday - Cached prices for weekend");
//     }
...
//     // Weekend: return cached Friday prices
//     if (isWeekendDay && Object.keys(weekendPriceCacheRef.current).length > 0) {
//       return weekendPriceCacheRef.current;
//     }

//     // ===== WEEKDAY: STABLE CHANGES =====
//     return {
//       "EUR/USD": {
//         price: bases["EUR/USD"],
//         change: realChangesRef.current["EUR/USD"]?.change ?? defaultChanges["EUR/USD"].change,
//         positive:
//           realChangesRef.current["EUR/USD"]?.positive ?? defaultChanges["EUR/USD"].positive,
//       },
//       "GBP/USD": {
//         price: bases["GBP/USD"],
//         change: realChangesRef.current["GBP/USD"]?.change ?? defaultChanges["GBP/USD"].change,
//         positive:
//           realChangesRef.current["GBP/USD"]?.positive ?? defaultChanges["GBP/USD"].positive,
//       },
//       "XAU/USD": {
//         price: bases["XAU/USD"],
//         change: realChangesRef.current["XAU/USD"]?.change ?? defaultChanges["XAU/USD"].change,
//         positive:
//           realChangesRef.current["XAU/USD"]?.positive ?? defaultChanges["XAU/USD"].positive,
//       },
//       "USD/JPY": {
//         price: bases["USD/JPY"],
//         change: realChangesRef.current["USD/JPY"]?.change ?? defaultChanges["USD/JPY"].change,
//         positive:
//           realChangesRef.current["USD/JPY"]?.positive ?? defaultChanges["USD/JPY"].positive,
//       },
//     };
//   };

//   // Binance WebSocket for BTC chart
//   useEffect(() => {
...
//   // Fallback simulation when WS disconnected
//   useEffect(() => {
...
//   // Twelve Data real prices (every 8 min) + real change %
//   useEffect(() => {
//     let isMounted = true;
...
//   // ===== Backup: purane names (agar API se kabhi aaye) =====
//   "SONIC SCALPER AI": {
//     subtitle: "Scalping Strategy",
//     icon: "🤖",
//     iconBg: "bg-blue-100 dark:bg-[#1e3a5f]",
...
//           </label>
//           <p className="mb-1.5 text-[11px] text-slate-500">Min: $100 | Max: $14,999</p>
//           <input
//             type="number"
//             step="1"
...
//               amountError ? "border-red-400 focus:ring-red-200" : "border-[#dfe6ef] focus:border-[#6ea8ff] focus:ring-[#b9d4ff]"
//             }`}
...
//   const { livePrice, priceChange, chartData, wsConnected, lastUpdate, marketPrices } =
//     useLiveMarket();
