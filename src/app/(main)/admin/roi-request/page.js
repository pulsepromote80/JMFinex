...
import {
    ...
    getAllUserROIWithdrawalRequest,
    updateROIWalletAdressUSDT
    
} from '@/app/redux/slices/fundManagerSlice'
import { upROIWithdReqStatusAdmin } from '@/app/redux/slices/fundManagerSlice'
import { usernameLoginId } from '@/app/redux/slices/adminMasterSlice'
import { toast } from 'react-toastify'
import { FaCopy, FaCalendarAlt, FaUser, FaIdBadge, FaSearch, FaFileExcel, FaSyncAlt, FaDollarSign, FaCheckCircle, FaTimesCircle, FaWallet, FaHistory } from 'react-icons/fa'
...
const WithdrawalRequest = () => {
    ...
    const { roiRequestData, loading, error } = useSelector(
    ...
    const handleSearch = () => {
        ...
        dispatch(getAllUserROIWithdrawalRequest(payload))
        setHasSearched(true)
    }

    const handleExport = () => {
        if (
            !roiRequestData?.unApWithROI ||
            roiRequestData?.unApWithROI?.length === 0
        ) {
        ...
        const worksheet = XLSX.utils.json_to_sheet(
            roiRequestData?.unApWithROI?.map((txn, index) => ({
                'Sr.No.': index + 1,
    ...
    const handleRefresh = () => {
        ...
        dispatch(getAllUserROIWithdrawalRequest({
            authLogin: '',
            fromDate: '',
            toDate: ''
        }))
    ...
    const totalRelease =
        hasSearched && roiRequestData?.unApWithROI ? roiRequestData.unApWithROI.reduce(
    ...
    const unApprovedRows = roiRequestData?.unApWithROI || []
            ...
            <div>
                ...
                <div className="mb-8 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <div className="bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-4">
                        ...
                        <p className="text-emerald-100 text-sm mt-1">Filter ROI withdrawal requests by date or user</p>
                    </div>
                    ...
                    <>
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-gray-800 rounded-2xl shadow-xl">
                                ...
                                <p className="mt-4 text-gray-500 dark:text-gray-400 font-medium">Loading ROI withdrawal requests...</p>
                            </div>
                        ) : error ? (
                                ...
                                <div className="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-emerald-50 dark:from-gray-900 dark:to-gray-800 border-b border-gray-200 dark:border-gray-700">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-sm font-medium text-gray-600 dark:text-gray-400">
                                            Found <span className="text-emerald-600 font-bold">{rowsToDisplay.length}</span> pending ROI requests
                                        </span>
                                    </div>
                                                ...
                                                <tr>
                                                    <td colSpan="14" className="px-6 py-12 text-center">
                                                        <div className="flex flex-col items-center gap-3">
                                                            ...
                                                            <p className="text-gray-500 dark:text-gray-400 font-medium">No ROI withdrawal requests found</p>
                                                        </div>
                                                    </td>
                                                </tr>
                                            ) : (
                                                paginatedRows.map((row, idx) => (
                                                    <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                                        ...
                                                        <td className="px-3 py-3 whitespace-nowrap">
                                                            ...
                                                            >
                                                                {processedRequests.has(row.Id) ? 'Approved' : 'Manual'}
                                                            </button>
                                                        </td>
